...
> **⚠️ 重要：绕过 macOS Gatekeeper 安全提示**
>
> 由于本项目尚未获得 Apple Developer 签名，macOS 可能会在首次打开时弹出 **”无法打开”** 或 **”文件已损坏”** 的警告。这**不是**真正的损坏，而是 macOS 对未签名应用的默认隔离策略。
>
> 请在将 App 拖入 `Applications` 文件夹后，打开**终端（Terminal）**，执行以下命令解除隔离：
>
> ```bash
> sudo xattr -rd com.apple.quarantine /Applications/RetroLaunchpad.app
> ```
>
> 输入你的 Mac 登录密码后回车即可。之后双击启动，即可享受 120Hz 的物理拖拽体验。
...
- [x] 抖动模式下的卸载逻辑与性能优化
- [x] 本地 JSON 布局记忆与动态同步
- [x] 防误触机制调优（卸载安全弹窗 + 层级退出）
- [x] 多显示器智能追踪与自适应
- [x] 原生设置窗口（开机自启 / 隐藏应用管理 / 布局重置）
- [x] 入场/退场深度动画（scale + blur + opacity spring）
